...
def registrationPage(loginWindowPanel):
    ...
    def registeredRecords(inchargeName, stnName, stnID, mobileNO, email, inchAddress, stationAddress, username, password):
    ...
    def validatePoliceRecords(inchargeName, stnName, stnID, mobileNO, email, inchAddress, stationAddress, username, password):
        getPoliceInchargeName = inchargeName
        getStationName = stnName
        getstationId = stnID
        getMobile = mobileNO
        ...
        if(len(getPoliceInchargeName) == 0 or len(getStationName) == 0 or len(getstationId) == 0 or len(getMobile) == 0 or len(getemail) == 0 or len(getemail) ==0 or len(getInchargeAddress) == 0 or len(getstationAddress) == 0 or len(getUsername) == 0 or len(getPassword) == 0):
            ...
            registeredRecords(inchargeName, stnName, stnID, mobileNO, email, inchAddress, stationAddress, username, password)            
        
    regCreateBtn.configure(command = lambda: validatePoliceRecords(policeentry.get(), stnEntry.get(), stnIDEntry.get(), mobEntry.get(), emailEntry.get(), inadsEntry.get(), stnAdsEntry.get(), usrEntry.get(), pswdEntry.get()))
...
def polManagePanel(username,password, inchargename,loginWindowPanel):
    ...
    def adddata(caseId,dob,criminalName,age,nickName,crimeDate,address,crimeType,birthMark,contactNo,fatherName,gender,mwanted,committedCrime,email):
        try:
            connection =  mysql.connector.connect(host = 'localhost',
                                                  database='criminalmanagement',
                                                  user = 'root',
    ...
    def validatePageFields(caseId,dob,criminalName,age,nickName,crimeDate,address,crimeType,birthMark,contactNo,fatherName,gender,mwanted,committedCrime,email):
        print("dob "+dob)
        ...
        if(caseId == '' or dob == '' or criminalName == '' or nickName == '' or crimeDate == '' or address == '' or crimeType == '' or birthMark == '' or contactNo == '' or fatherName == '' or committedCrime == '' or email == ''):
            ...
            return 0
        adddata(caseId,dob,criminalName,derAge,nickName,crimeDate,address,crimeType,birthMark,contactNo,fatherName,gender,mwanted,committedCrime,email)
    ...
    savebtn = Button(criminalPanel,
                     text="Save",
                     font=("Times New Roman",15),
                     command=lambda:validatePageFields(caseIdEntry.get(),dobpicker.get(),crmNameEntry.get(),derAgeLbl,nickNameEntry.get(),crimedatePicker.get(),addressEntry.get(),crimeTypeEntry.get(),birthMarkEntry.get(),contactEntry.get(),fatherEntry.get(),gendervar.get(),mwantedvar.get(),committedCrimeEntry.get(),emailEntry.get()))
...
def modificationPage(username,inchargename):
    ...
    def updateRecord(caseId,dob,criminalName,derAge,nickname,crimeDate,address,crimeType,birthmark,contactNo,fatherName,gender,mWanted,committedCrime,email):
        try:
            connection = mysql.connector.connect(host = 'localhost',
                                                 database='criminalmanagement',
                                                 user='root',
    ...
    def validatePage(caseId,dob,criminalName,age,nickName,crimeDate,address,crimeType,birthMark,contactNo,fatherName,gender,mwanted,committedCrime,email):
        print("dob "+dob)
        dob_date = datetime.strptime(dob,'%d-%m-%Y')
        cur_date = datetime.now()
        year1 = cur_date.year
        ...
        if(caseId == '' or dob == '' or criminalName == '' or nickName == '' or crimeDate == '' or address == '' or crimeType == '' or birthMark == '' or contactNo == '' or fatherName == '' or committedCrime == '' or email == ''):
            ...
            messagebox.showerror("Error","Contact number number must be numbered value")
            return 0
        updateRecord(caseId,dob,criminalName,derAge,nickName,crimeDate,address,crimeType,birthMark,contactNo,fatherName,gender,mwanted,committedCrime,email)
    ...
    updatebtn = Button(modifiedPanel,
                     text="Update",
                     font=("Times New Roman",15),
                     command=lambda: validatePage(clicked.get(),dobpicker.get(),crmNameEntry.get(),derAgeLbl,nickNameEntry.get(),crimedatePicker.get(),addressEntry.get(),crimeTypeEntry.get(),birthMarkEntry.get(),contactEntry.get(),fatherEntry.get(),gendervar.get(),mwantedvar.get(),committedCrimeEntry.get(),emailEntry.get()))
